      ...
      respond_to do |format|
        ...
        format.csv do
          ...
          send_data(query_to_csv(@issues, @query, params[:csv]),
                    :type => 'text/csv; header=present', :filename => "#{filename_for_export(@query, 'requests')}.csv")
            end
    
          @issues = @query.issues(:limit => Setting.issues_export_limit.to_i)
          send_file_headers! :type => 'application/pdf', :filename => "#{filename_for_export(@query, 'requests')}.pdf"
        end
      end
    else
      respond_to do |format|
